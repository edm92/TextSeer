...
public class Accumulate implements Serializable{
	...
	/**
	 * Do pairwise accumulation and return the set of maximally consistent WFFs that result from accumulation
	 * @param source starting WFF
	 * @param target new WFF to accumulate with
	 * @param KB existing knowledgebase
	 * @return
	 */
	@SuppressWarnings("unchecked")
	private LinkedHashSet<WFF> pairwise_acc_old(WFF source, WFF target, String KB){
     	if(!source.eval(target, KB)){ // Not sat so compute maxSubsets
	     	try{
	     		...
	     		do{
		     		...
		     		Set<WFF> correctDepthResults = makeCorrectDepthResults(source);
		     		...
		     		for(WFF newResult : correctDepthResults){
		     			...
		     			//for(WFF e : returnPairResults) System.out.println(currentDepth + ") Returned on " + e );
	     		...
	     		//for(WFF e : results) System.err.println(currentDepth + ") Working on " + e);
     		...
     		WFF resultingScenario = new WFF(source.getEffect(target.getFormula(), KB));
	...
	private Set<WFF> makeCorrectDepthResults(WFF source) {
		...
		try{
	        ...
	        PartitionListElement<Clause> reducedWFF = new PartitionListElement<Clause>();
	    	for(Clause cls :(Set<Clause>) myClauses){
	    		reducedWFF.add(cls);
			}
	    	
	    	int[] indices;
	        CombinationGenerator x = new CombinationGenerator (reducedWFF.size(), currentDepth);
	    	...
	    	for(String formString: myWFF){
	    		...
	    		//System.out.println("Made new WFF at depth " + currentDepth + " = " + newForm);
	...
	@SuppressWarnings("unchecked")
	private LinkedHashSet<WFF> rec_pair_acc(WFF sourceWFF,
			WFF targetWFF, String KB, boolean Loop) {
		...
		if(sourceWFF.eval(targetWFF,KB)){
			...
			//System.err.println("Eval of " + sourceWFF + " and " + targetWFF + " with " + KB + " failed!");
		}
		...
		try{
	        ...
	        DefaultClausalFactory myFacts = new DefaultClausalFactory();
	        ClausalSet myClauses =  myFacts.asClausalSet(result);
	        if(myClauses.size() <= 1) {	resultsList.add(targetWFF); return resultsList;} // return the target and still no matches
	    	for(Clause cl :(Set<Clause>) myClauses){	// For each clause in current WFF, remove an element
	    		String newForm = "";
	    		for(Clause cls :(Set<Clause>) myClauses){
	    		...
	    		if(newWFF.eval(targetWFF,KB))
	    			...
	    			//System.err.println("Adding " + newWFF + " to nextLevellist");
				...
				for(WFF newSource: nextLevelList){
					if(!explored.contains(newSource.getFormula())){
						if(numClauses(newSource) == currentDepth){
							...
							resultsList.addAll(rec_pair_acc(newSource, targetWFF, KB, distro));
						}
					}
...
//	@SuppressWarnings("unchecked")
//	private LinkedHashSet<WFF> getMaxSubsets(WFF sourceWFF, WFF targetWFF, String KB){
