...
/**
 * POSET From wikipedia - A (non-strict) partial order is a binary relation "≤"
 * over a set P which is reflexive, antisymmetric, and transitive, i.e., which
 * satisfies for all a, b, and c in P: 1) a ≤ a (reflexivity); 2) if a ≤ b and b
 * ≤ a then a = b (antisymmetry); 3) if a ≤ b and b ≤ c then a ≤ c
 * (transitivity).
 * 
 * The use of this class is pretty simple, declare a new POSET Poset p = new
 * Poset(); Then add as many elements as you like to the list (this is a type
 * driven base function) p.List( P(anything, anything) , ... );
 * 
 * p.List(P("a", "b"), P("b","c"), P("a","d"), P("d","c"));
 * 
 * The the functions lt (less than) and leq (less than equal to) are available
 * to you for use.
 * 
 * For a copy of the literate description of this function see
 * https://sites.google
 * .com/a/fnord.be/flight-of-the-fnord/blog/literatejavaposets/java-poset.pdf
 * 
...
public class Poset {
	...
	public <T> Map<T, HashSet<T>> computeOrders(List<Pair<T, T>> s) {
		...
		for (Pair<T, T> p : s) {
			if (orders.containsKey(p.getFirst())) {
				((HashSet<T>) orders.get(p.getFirst())).add(p.getSecond());
			} else {
			...
			for (T ele : (HashSet<T>) orders.get(key)) {
				if (((TreeMap<T, HashSet<T>>) backwards).containsKey(ele)) {
					...
					if (((TreeMap<T, HashSet<T>>) backwards).containsKey(key)) {
						((TreeMap<T, HashSet<T>>) backwards).get(ele).addAll(
	...
	public <T> boolean leq(T a, T b) {
		...
		if (backwards != null
				&& (backwards.containsKey(a) || backwards.containsKey(b))) {
			if (backwards.containsKey(b)
	...
	public <T> Map<T, HashSet<T>> List(T... elements) {
		return computeOrders((java.util.List<Pair<T, T>>) new LinkedList<T>(
