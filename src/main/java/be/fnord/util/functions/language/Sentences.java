...
public class Sentences {
	public enum ALG {
		SIMPLE_COMPARE_ALG, SIMPLE_COMPARE_WITH_MANYTOONE;
	}
	...
	// public double MIN_MATCH_SENTENCE_SCORE =
	...
	public static void main(String[] args) {
		...
		LinkedList<String> Proc1 = s.List("task1", "task two");
		...
		LinkedList<String> Proc2 = s.List("task two", "task one");
		...
		Pair<String, String> myNewSentences = s
				.CreateStringOfTwoSetsOfSentences(Proc1, Proc2,
		...
		DamerauLevenshtein d = new DamerauLevenshtein(
				myNewSentences.getFirst(), myNewSentences.getSecond());
	...
	public String Clean(String in, boolean SHORT_STOPWORDS) {
		...
		if (in.contains("_")) {
			...
			in = in.trim();
			if (in.substring(0, 1).matches("[0-9]")) {
		...
		// Remove start exclusive and parallel
		in = in.replaceAll("Exclusive Gateway", "")
				.replaceAll("Start Event", "").replaceAll("End Event", "")
				.replaceAll("Event", "");
		String _result = "";
		...
		for (String s : Tokenize(in, SHORT_STOPWORDS)) {
			_result += s + " ";
		}
		// if(_result.length()>0) _result = _result.substring(0,
		// _result.length() -1);
	...
	LinkedList<String> Proc1, LinkedList<String> Proc2, ALG ALG_TO_RUN,
			double fixNumber) {
		double MIN_MATCH_SENTENCE_SCORE = fixNumber;
		// HashMap<String, LinkedList<String>> result = new HashMap<String,
		// LinkedList<String>> ();
		...
		for (String str : Proc1) {
			String cleaned = Clean(str, false);
		...
		HashMap<String, LinkedList<SimSet>> hm = DoSimCheck(cleanProc1,
				cleanProc2, MIN_MATCH_SENTENCE_SCORE);
		hm = fixTwoProcSimSet(hm, cleanProc1, cleanProc2);

		HashMap<String, String> simple = null;
		if (ALG_TO_RUN == ALG.SIMPLE_COMPARE_ALG) {
			simple = simpleMatchClosest(hm, cleanProc1, cleanProc2,
					!INCLUDE_MANY_TO_ONE_MATCHES);
		} else if (ALG_TO_RUN == ALG.SIMPLE_COMPARE_WITH_MANYTOONE) {
			simple = simpleMatchClosest(hm, cleanProc1, cleanProc2,
					INCLUDE_MANY_TO_ONE_MATCHES);
		}
		...
		for (String str : simple.keySet()) {
			...
			if (!revCharMapping.containsKey(str)) {
				...
				if (revCharMapping.containsKey(alt)) {
					...
					chr = a.e.RANDOM_RANGE[currentRange++];
				}
		...
		Pair<String, String> _result = new Pair<String, String>(_result1,
				_result2);
		// LinkedList<String> removeList = new LinkedList<String>();
		// for(String str : simple.keySet()){
	...
	public HashMap<String, LinkedList<SimSet>> DoSimCheck(
			LinkedList<String> _sentencesA, LinkedList<String> _sentencesB,
			Double MATCH_SCORE) {
		...
		for (String __str : _sentencesA) {
			_a = Clean(__str, false);
			for (String _str : _sentencesB) {
				if (__str.compareToIgnoreCase(_str) == 0) {
					continue;
				}
				// if(!started && _a.length() < 1) { _a = _str ; started = true;
				// continue;};
				String _b = _str;

				// Process pair _a and _b
				String __a = _a;
				String __b = Clean(_b, false);
				...
				if (result >= MATCH_SCORE) { // Save
					...
					if (LLS != null) {
						for (SimSet sims : LLS) {
							if (sims.sent2.compareToIgnoreCase(__b) == 0) {
								breaker = true;
							}
						}
						if (!breaker) {
						...
						LLS = new LinkedList<SimSet>();
						LLS.add(ss);
						SimSents.put(__a, LLS);
					}

				}
				// a.e.println("Result of comparing remaining = " +
				// SentenceSim(__a,__b) + " (" + __a + ", " + __b +")");
	...
	public HashMap<String, LinkedList<SimSet>> fixTwoProcSimSet(
			HashMap<String, LinkedList<SimSet>> _in, LinkedList<String> proc1,
	...
	public <T> LinkedList<String> List(T... elements) {
		LinkedList<String> sentences = (LinkedList<String>) new LinkedList<T>(
	...
	public double SentenceSim(String _a, String _b, double MATCH_SCORE) {
		...
		ArrayList<String> _aList = Tokenize(_a, false);
		ArrayList<String> _bList = Tokenize(_b, false);
		// double words = ((_aList.size() > 0 ? _aList.size() : 1) +
		// (_bList.size() > 0 ? _bList.size() : 1)) / 2;
		for (String WFFL : _aList) {
			for (String WFSL : _bList) { // Word from second list
				double currentBest = sim.getSim(WFFL, WFSL);
				// a.e.println("Result of " + WFFL + " vs. " + WFSL + " = " +
				// currentBest);
	 ...
	 * @param _in
	 * @param MANYTOONE
	 *            Give either INCLUDE_MANY_TO_ONE_MATCHES or
	 *            !INCLUDE_MANY_TO_ONE_MATCHES
	 * @return
	 */
	public HashMap<String, String> simpleMatchClosest(
			HashMap<String, LinkedList<SimSet>> _in,
			LinkedList<String> _sentences1, LinkedList<String> _sentences2,
			boolean MANYTOONE) {
		...
		for (String key : _in.keySet()) {
			...
			if (_result.containsKey(key) || _result.containsKey(sBest)) {
				// a.e.println("Found overlap " + key + " " + sBest); // todo
				// Fix here
				taken.add(sBest);
		...
		for (String s : _result.keySet()) {
			...
			if (_result.get(s).trim().length() < 2) {
				remove.add(s);
			}
		}
		for (String s : remove) {
	...
	public ArrayList<String> Tokenize(String in, boolean SHORT_STOPWORDS) {
		ArrayList<String> tokens = new ArrayList<String>();
		// create a new instance of PorterStemmer
		// PorterStemmer stemmer = new PorterStemmer();

		// Turn off Stanford loggin -- Thanks to
		// http://stackoverflow.com/questions/21851217/how-to-shutdown-stanford-corenlp-redwood-logging
