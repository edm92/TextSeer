...
public class WordSim {
	...
	/**
	 * Should return a score of similarity between 0 and 1. 1 is similar and 0
	 * is not similar
	 * 
	 * @param _a
	 * @param _b
	...
	public double getSim(String _a, String _b) {
		...
		for (String s : types) {
			try {
				...
				double score = rc.calcRelatednessOfSynset(
						mySyms.getFirst().get(0), mySyms.getSecond().get(0))
						.getScore();
				if (currentBest < score) {
					currentBest = score;
					// a.e.println("Sim = " +
					// rc.calcRelatednessOfSynset(mySyms.getFirst().get(1),
					// mySyms.getSecond().get(0)).getScore()) ;
					// a.e.println("Sim = " + score +" " + currentBest + " " +
					// _a + " " + _b + ":" + s);
				}
			} catch (Exception e) {
				//
	...
	public Pair<List<Concept>, List<Concept>> MakeSyns(String _a, String _b,
			String _type) {
		List<Concept> _n1Synsets = toSynsets(_a, _type);

		List<Concept> _n2Synsets = toSynsets(_b, _type);
		// a.e.println("_n1SynSet for " + _a + "(" + _type + ") is " +
		// _n1Synsets.toString());
		// a.e.println("_n1SynSet for " + _b + "(" + _type + ") is " +
		// _n2Synsets.toString());
		Pair<List<Concept>, List<Concept>> _result = new Pair<List<Concept>, List<Concept>>(
				_n1Synsets, _n2Synsets);
		return _result;
	}
