...
public class Accumulate_4ST extends EffectAccumulate {
	 ...
	 *         stored
	 */
	public Graph_ST<Vertex_ST, Edge_ST> trace_acc(Trace src, String kb,
			Graph_ST<Vertex_ST, Edge_ST> gST) {
		...
		for (Vertex v : src.getNodes()) { // traverse nodes in the given trace
			...
			if (src.getNodes().get(src.getNodes().size() - 1).equals(v)
				...
				for (WFF e : intEff) {
					...
					tmpV.add(new Vertex_ST(v.name, v.getWFF().toString(), e
							.toString())); // add
					// candidate
					// vertex
					// to
		...
		for (int i = 1; i < vList.size(); i++) {
			for (int j = 0; j < vList.get(i).size(); j++) {
				for (int x = 0; x < vList.get(i - 1).size(); x++) {
					...
					LinkedHashSet<WFF> effCheckList = accCheck.pairwise_acc(
							new WFF(vList.get(i - 1).get(x).esWFF), new WFF(
