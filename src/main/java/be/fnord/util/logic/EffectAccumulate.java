...
public class EffectAccumulate implements Serializable {
	...
	//
	// String tmp_intEff;
	// tmp_intEff = intEff.toString().replaceAll("\\[",
	// "").replaceAll("\\]","");
		...
		try {
			...
			while (x.hasMore()) {
				...
				for (int indice : indices) {
					combination.append(reducedWFF.get(indice));
				}
	...
	public LinkedHashSet<WFF> pairwise_acc(WFF source, WFF target, String KB,
			boolean distrobute) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private LinkedHashSet<WFF> pairwise_acc_old(WFF source, WFF target,
			String KB) {
		...
		if (!source.eval(target, KB)) { // Not sat so compute maxSubsets
			try {
				...
				do {
					...
					for (WFF newResult : correctDepthResults) {
						...
						returnPairResults = rec_pair_acc(newResult, target, KB,
								distro);
				...
				for (WFF e : results) {
					...
					formula = (Formula) logic.createExpression("("
							+ e.getFormula() + ") & (" + target.getFormula()
							+ ")");
			...
			WFF resultingScenario = new WFF(source.getEffect(
					target.getFormula(), KB));
	...
	@SuppressWarnings("unchecked")
	private LinkedHashSet<WFF> rec_pair_acc(WFF sourceWFF, WFF targetWFF,
			String KB, boolean Loop) {
			...
			for (WFF newSource : nextLevelList) {
				if (!explored.contains(newSource.getFormula())) {
					if (numClauses(newSource) == currentDepth) {
						...
						resultsList.addAll(rec_pair_acc(newSource, targetWFF,
								KB, distro));
					}
				}
	...
	public LinkedHashSet<WFF> trace_acc(Trace src, String kb) {
		...
		for (Vertex v : src.getNodes()) {
			...
			for (WFF ce : currentEff) {
				if (!v.getWFF().isEmpty()) {
					intEff.remove(ce);
					EffectAccumulate acc = new EffectAccumulate(); // Needed
																	// because
																	// of
					// semi-static variables
					intEff.addAll(acc.pairwise_acc(ce, v.getWFF(), kb, true));
					if (intEff != null) {
						v.cumWFF.addAll(intEff); // Copy current intimediate
													// effect to our cummulative
													// effects
					}
