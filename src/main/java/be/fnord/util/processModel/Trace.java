...
public class Trace extends Vertex {
	 ...
	 * Technically it speeds up processing a little, but will also screw with
	 * parallel processing
	 */
	...
	public LinkedList<LinkedList<Vertex>> _cleanVertexArray(
			LinkedList<LinkedList<Vertex>> input) {
	...
	public LinkedList<Vertex> _processGroupsOfParallelNodes(
			LinkedList<Vertex> currentLL) {
	...
	public LinkedList<LinkedList<Vertex>> _toVertexArray(
			boolean SHOULD_IGNORE_START_END_NODES) {
		...
		for (Vertex v : nodes) {
			...
			if (v.isTrace) { // It is, so lets process it as a trace.
				// In square is used to group traces next to each other, i.e.
				// <a, [<b,c>,<d,e>],f >
				// for the group in the square <b,c>,<d,e>, the group should be
				// OCP'd together (to get the result:
				// <b,c,d,e>, <b,d,c,e>, <b,d,e,c>, <d,b,c,e> ... etc.
				if (!inSquares) {
					inSquares = true;
				}
				Trace t = (Trace) v; // We expand the this trace into a list of
										// lists of elements.
				LinkedList<LinkedList<Vertex>> newvArray = t
						._toVertexArray(IGNORE_START_END_NODES_IN_TRACE);
				...
				if (inSquares) {
					...
					newResult.addAll(returnedResults); // .substring(0,
	...
	public LinkedList<LinkedList<Vertex>> processGroupsOfParallelNodes(
			LinkedList<LinkedList<Vertex>> currentLL) {
		...
		for (LinkedList<Vertex> cc : currentLL) {
			...
			int numIterations = 0;
			PartitionList<Vertex> OCPresults1 = null; // OrderConstrainedPartitionList.makePartitions(
														// list);
			...
			for (PartitionListElement<Vertex> _list : OCS) {
				...
				case 0:
					OCPresults1 = OrderConstrainedPartitionList
							.makePartitions(_list);
					numIterations++;
					break;
				case 1:
					OCPresults2 = OrderConstrainedPartitionList
							.makePartitions(_list);
					OCPresults3 = OrderConstrainedPartitionList.joinSets(
				...
				default:
					OCPresults2 = OrderConstrainedPartitionList
							.makePartitions(_list);
					OCPresults3 = OrderConstrainedPartitionList
							.joinPartitionedSets(OCPresults1, OCPresults2);
					// Shuffle back
					OCPresults1 = OCPresults2;
					OCPresults2 = OCPresults3;
					break;
	...
	private Trace processSimpleSubTraces(LinkedList<Vertex> vertexArray) {
		...
		newTrace.isAND = isAND;
		newTrace.MY_ID = MY_ID;

		addSubTrace(newTrace.name);
		...
		for (Vertex v : vertexArray) {
			...
			// Add in the edges
			if (previous != null) {
				Edge e = parentGraph.getEdge(previous, v);
	...
	/**
	 * Once we are happy with our trace, we will create subtraces which are the
	 * actual <v,v,v,v> traces
	 *
	 * @param SIMPLE_OR_FULL
	 *            either SIMPLE_TRACES or FULL_TRACES
	 * @return
	 */
	...
	/**
	 * toVertexArray should take as input a trace and then convert that trace
	 * into a list of verticies. The actual return is a list of lists of
	 * vertices. We have multiple returns because for each array conduct an
	...
	public LinkedList<LinkedList<Vertex>> toVertexArray() {
		...
		Trace.reset(); // This is a really weird hack and will screw with
						// parallel processing.
		// Needed because for some reason the processGroupsOfParallelNodes
