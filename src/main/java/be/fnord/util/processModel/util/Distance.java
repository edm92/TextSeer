...
public class Distance {
	...
	class SimilarPair {
		...
		public String toString() {
			...
			a.e.decIndent();
			result += "Score: " + simScore + "; id = " + first_id + ","
					+ second_id;
	...
	public static void main(String[] args) {
		...
		double distance = 0;
		// distance =
		// dc.computeDistance("models/Determine environmental health and safety impacts.bpmn20.xml",
		// "models/proc3.bpmn20.xml",
		// ALG.SIMPLE_COMPARE_ALG, WORD_MATCH_STRENGTH.EXACT, SIM_RESULT.RATIO);
		distance = dc.computeDistance("models/LoanApp/CallLogger2.bpmn20.xml",
				"models/LoanApp/LoanApp2.bpmn20.xml", ALG.SIMPLE_COMPARE_ALG,
				WORD_MATCH_STRENGTH.EXACT, SIM_RESULT.WHOLE_NUMBER);
		a.e.println("Distance between the two models is " + distance);

		// distance = dc.computeDistance("models/sim/proc3.bpmn20.xml",
		// "models/sim/proc1.bpmn20.xml",
		// ALG.SIMPLE_COMPARE_ALG, WORD_MATCH_STRENGTH.EXACT, SIM_RESULT.RATIO);
		// a.e.println("Distance between the two models is " + distance);

	}

	public double computeDistance(Graph<Vertex, Edge> g1,
			Graph<Vertex, Edge> g2, ALG ALG_TYPE,
			WORD_MATCH_STRENGTH MATCHSTRENGH, SIM_RESULT RAT_OR_WHO) {
		double _distance = 0;
		GraphChecker gc1 = new GraphChecker();
		GraphChecker gc2 = new GraphChecker();
		if (!gc1.CheckGraph(g1)) {
			if (__DEBUG) {
				a.e.err("1Error in graph + " + g1.name + " aborting.");
			}
			return 0;
		}
		;
		if (!gc2.CheckGraph(g2)) {
			if (__DEBUG) {
					a.e.err("2Error in graph + " + g2.name + " aborting.");
				}
			return 0;
		}
		;
		if (!gc1.CheckEventsAndGateways(g1)) {
				a.e.println("Issue checking events and gateways");
			}
		if (!gc2.CheckEventsAndGateways(g1)) {
				a.e.println("Issue checking events and gateways");
			}
		...
		LinkedList<Graph<Vertex, Edge>> _decisionless1 = gt1
				.makeDecisionFree(g1);
		LinkedList<Graph<Vertex, Edge>> decisionless1 = gt1
				.removeDupesFromDecisionFreeGraphs(_decisionless1);

		LinkedList<Graph<Vertex, Edge>> _decisionless2 = gt2
				.makeDecisionFree(g2);
		LinkedList<Graph<Vertex, Edge>> decisionless2 = gt2
				.removeDupesFromDecisionFreeGraphs(_decisionless2);
		...
		if (traces2 != null && traces1 != null) {
				_distance = distance(traces1, traces2, ALG_TYPE, MATCHSTRENGH,
					RAT_OR_WHO);
			}
	...
	public double computeDistance(String file1, String file2, ALG ALG_TYPE,
			WORD_MATCH_STRENGTH MATCHSTRENGH, SIM_RESULT RAT_OR_WHO) {
		GraphLoader gLoader = new GraphLoader();
		Graph<Vertex, Edge> g1 = gLoader.loadModel(file1,
				a.e.DONT_SAVE_MESSAGES_AND_PARTICIPANTS);
		Graph<Vertex, Edge> g2 = gLoader.loadModel(file2,
				a.e.DONT_SAVE_MESSAGES_AND_PARTICIPANTS);
	...
	public double distance(LinkedList<Trace> _a, LinkedList<Trace> _b,
		...
		do {
			...
			SimilarPair mostSim = getMostSimilar(sentences1, sentences2,
					ALG_TYPE, MATCHSTRENGH, RAT_OR_WHO);
			if (__DEBUG) {
			...
			for (LinkedList<String> s : sentences1) {
				...
				if (__DEBUG) {
					a.e.println("Last " + last);
				}
				if (last != null && last.trim().equals(mostSim.first_id + "")) {
					remove.add(s);
				}
			}
			...
			for (LinkedList<String> s : sentences2) {
				...
				if (last != null && last.trim().equals(mostSim.second_id + "")) {
					remove.add(s);
				}
			}
			for (LinkedList<String> ss : remove) {
			...
			if (sentences1.size() < 2 || sentences2.size() < 2) {
				loopBreaker = loopBreaker > 1 ? 1 : loopBreaker;
			}
	...
	private SimilarPair getMostSimilar(
			LinkedList<LinkedList<String>> sentences1,
			LinkedList<LinkedList<String>> sentences2, ALG ALG_TYPE,
			WORD_MATCH_STRENGTH MATCHSTRENGH, SIM_RESULT RAT_OR_WHO) {
		...
		for (LinkedList<String> _fsentences : sentences1) {
			for (LinkedList<String> _ssentences : sentences2) {
				...
				int id2 = 0;
				try {
					id1 = Integer.parseInt(_fsentences.getLast());
					_fsentences.removeLast();
					id2 = Integer.parseInt(_ssentences.getLast());
					_ssentences.removeLast();
				} catch (Exception e) {
				...
				Pair<String, String> myNewSentences = s
						.CreateStringOfTwoSetsOfSentences(_fsentences,
								_ssentences, ALG_TYPE,
				...
				for (char i : myNewSentences.getFirst().toCharArray()) {
					for (char j : myNewSentences.getSecond().toCharArray()) {
						if (i == j) {
							...
							numChar -= a.e.PENALTY_FOR_EXTRA_LETTER;
						}
					}
				}
				if (numChar < 1) {
				...
				if (__DEBUG) {
					a.e.println("Distance between " + myNewSentences.getFirst()
							+ " and " + myNewSentences.getSecond() + " is "
							+ sim + " - Char Dif =" + numChar + " (" + oldSim
							+ ") " + (1 - sim));
				}
