...
//        		ALG.SIMPLE_COMPARE_ALG, WORD_MATCH_STRENGTH.EXACT, SIM_RESULT.RATIO);
        distance = dc.computeDistance("models/LoanApp/CallLogger2.bpmn20.xml", "models/LoanApp/LoanApp2.bpmn20.xml",
...
//        a.e.println("Distance between the two models is " + distance);
	...
	public double computeDistance(Graph<Vertex,Edge> g1, Graph<Vertex,Edge> g2, 
        ...
        if(!gc1.CheckGraph(g1)) { 
        	if(__DEBUG) a.e.err("1Error in graph + " + g1.name + " aborting."); 
        	return 0; };
        if(!gc2.CheckGraph(g2)) { 
        	if(__DEBUG) a.e.err("2Error in graph + " + g2.name + " aborting."); 
        	return 0; 
        	};
