...
/**
 * The following class is used to check the well formedness of a graph. The main
 * method CheckGraph will also modify the graph as best as possible to 'fix' any
 * issues that would cause other functions to barf. After loading a graph it
 * should be run through the Check class.
 *
...
public class GraphChecker {
	...
	public static final int MAX_PATH_LENGTH = 100; // Cut off point for
													// processing gateway fixes
	public static final int MAX_TIMES_TRY_FIX = 1; // Cut off point for fixing
													// gateway issues
	...
	public boolean CheckEventsAndGateways(Graph<Vertex, Edge> g) {
		...
		for (Vertex v : g.vertexSet()) {
			...
			// Check if there is a boundary and create an edge between the node
			// and the boundary
			...
			// Convert OR's to and/XORS -- Analyst note, should not model with
			// OR's
		...
		for (Vertex v : multiOuts) {
			Vertex v2 = new Vertex("subGate" + UUID.randomUUID(),
		...
		for (String u : unconnectedNodes) {
			...
			if (__INFO) {
				a.e.println("Removed unconnected node: " + u);
			}
		}
			...
			if (!(v.isAND || v.isXOR || v.isOR)) {
				if (g.inDegreeOf(v) > 1 || g.outDegreeOf(v) > 1) {
					if (__DEBUG) {
						a.e.println("Vertex "
								+ v.name
								+ " has a large in/out degree for a non-gateway, converting to XOR");
					}
					fixXORs.add(v);
				...
				if (g.inDegreeOf(v) > 1 && g.outDegreeOf(v) > 1) {
					if (__DEBUG) {
						a.e.println("Gateway " + v.name
								+ " is used as both a split and a join.");
					}
					fixDoubles.add(v);
					// removed return false
				}
		...
		for (Vertex v : fixXORs) {
			if (g.outDegreeOf(v) > 1) {
				Vertex v2 = new Vertex("newXORGate-" + UUID.randomUUID(),
						GraphLoader.ExclusiveGateway);
				v2.isXOR = true;
				v2.isGateway = true;
				v2.isSubstructural = true;
		...
		for (Vertex v : g.vertexSet()) {
			...
			if (v.isSplit && v.isJoin) {
				a.e.println("GraphChecker.CheckEventsAndGateways failed to fix splits and joins on "
						+ v.toString());
				return false;
			}
		}
	...
	/**
	 * Check the graph for good structures, fix some if possible and remove the
	 * rest This function will add a substructural start and end node to the
	 * process which means that the process will only end up with a single start
	 * and a single end.
	 *
	 * @return True if good structured model, false if badly structured model
	 *         beyond repair
	 */
	public boolean CheckGraph(Graph<Vertex, Edge> g) {
		// Fix the gateways and boundaries first
		if (!CheckEventsAndGateways(g)) {
			return false;
		}

		// This includes a parallel gateway for splitting multiplestarts
		if (startNodes.size() > 1) {
			Vertex newStart = new Vertex("newStart-" + UUID.randomUUID(),
					GraphLoader.ParallelGateway);
			newStart.isAND = true;
			...
			g.trueStart = g.vertexRef.get(startNodes.get(0));
		}

		if (endNodes.size() > 1) {
			Vertex newEnd = new Vertex("newEnd-" + UUID.randomUUID(),
					GraphLoader.ParallelGateway);
			newEnd.isAND = true;
			newEnd.isJoin = true;
			...
			if (g.trueStart.type == GraphLoader.ParallelGateway) {
				g.trueStart.setCorresponding(newEnd);
			}
			for (String s : startNodes) {
				// Create edge to each node
			...
			g.trueEnd = g.vertexRef.get(endNodes.get(0));
		}
		// Do cleanup (add in missing gateway to start of process)
		if (g.trueEnd.type == GraphLoader.ParallelGateway
				&& g.trueEnd.corresponding == null) {
			Vertex newStartGate = new Vertex("newStartGate-"
					+ UUID.randomUUID(), GraphLoader.ParallelGateway);
			newStartGate.isAND = true;
			newStartGate.isSplit = true;
			newStartGate.isSubstructural = true;
			newStartGate.setCorresponding(g.trueEnd);
			g.addV(newStartGate);
			// Replace edges from the start node to vertices and recreate from
			// the new start node gateway
		...
		if (g.trueStart.type == GraphLoader.ParallelGateway
				&& g.trueStart.corresponding == null) {
			Vertex newEndGate = new Vertex("newEndGate-" + UUID.randomUUID(),
					GraphLoader.ParallelGateway);
		...
		StrongConnectivityInspector<Vertex, Edge> sci = new StrongConnectivityInspector<Vertex, Edge>(
				copy);
	...
	private void fixCorrespondingGates(Graph<Vertex, Edge> g) {
		...
		KShortestPaths<Vertex, Edge> sp = new KShortestPaths<Vertex, Edge>(g,
				g.trueStart, MAX_PATH_LENGTH);
	...
	/**
	 * Search graph for corresponding gateways and attempt to fix small
	 * structural issues
	 *
	 * @param g
	 *            input Graph
	 * @return return true if no errors needed fixing and false if there were
	 *         some errors found
	 */
	...
	private boolean fixGateways(Graph<Vertex, Edge> g) {
		...
		if (++NumberFixed < MAX_TIMES_TRY_FIX) {
			fixGateways(g);
		}
		return result;
	}
	...
	private void fixLessCloseGates(Graph<Vertex, Edge> g) {
		...
		for (GraphPath<Vertex, Edge> gp : sp.getPaths(g.trueEnd)) {
			...
			for (String currentGate : gateways) {
				...
				if (!(scoreTallyOut.get(last) == null || scoreTallyIn
						.get(currentGate) == null)) {
					if (scoreTallyOut.get(last) > scoreTallyIn.get(currentGate)) {
						if (__INFO) {
							a.e.println("Found point of interest: " + last
									+ ":" + scoreTallyOut.get(last) + " vs. "
									+ currentGate + ":"
									+ scoreTallyIn.get(currentGate));
						}
					}// else System.err.println("No point of interest: " + last
						// + ":" + scoreTallyOut.get(last) + " vs. " +
						// currentGate + ":" + scoreTallyIn.get(currentGate));
				}
				last = currentGate;
			}
		}
	...
	private void loopyfix(LinkedList<String> gateways,
			...
			TreeMap<String, Integer> scoreTallyOut, Graph<Vertex, Edge> g) {
		if (gateways.size() < 1) {
			return;
		}
			...
			if (scoreTallyOut.get(last) != null && scoreTallyIn.get(s) != null) {
				if (scoreTallyOut.get(last) > 1 && scoreTallyIn.get(s) > 1) {
					...
					if (pntSt != null && pntEd != null) {
						// System.err.println("Setting correspoinding" + pntSt +
						// "," + pntEd);
		...
		if (contList.size() > 1) {
			loopyfix(contList, scoreTallyIn, scoreTallyOut, g);
		}

	}

	private Vertex next(Vertex current, GraphPath<Vertex, Edge> gp,
			Graph<Vertex, Edge> g) {
	...
	private boolean testGateways(Graph<Vertex, Edge> g) {
		...
		for (Vertex v : g.vertexSet()) {
			// Count number of gateways first
			if (v.isGateway) {
				if (v.getCorresponding() == null && !v.isXOR) { // Check if we
																// labeled a
																// corresponding
																// gateway
					if (__DEBUG) {
						a.e.println("Didn't find a corresponding gate for "
								+ v.toString());
					}
					return false;
				} else {
					if (!v.isXOR) {
						if (v.getCorresponding().getCorresponding().id
								.toString().compareTo(v.id.toString()) != 0) {
							if (__DEBUG) {
								a.e.println("Corresponding gate has been associated to something else "
										+ v.toString()
										+ "; "
										+ v.getCorresponding().toString());
							}
							if (__DEBUG) {
								a.e.println("Corresponding gate has been associated to something else "
										+ v.getCorresponding().toString()
