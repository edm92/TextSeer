...
public class GraphLoader {
	...
	public static final int BoundaryEvent = "org.yaoqiang.bpmn.model.elements.events.BoundaryEvent"
			.hashCode();
	public static final int BPMNDiagram = "org.yaoqiang.bpmn.model.elements.bpmndi.BPMNDiagram"
			.hashCode();
	public static final int BPMNEdge = "org.yaoqiang.bpmn.model.elements.bpmndi.BPMNEdge"
			.hashCode();

	public static final int BPMNPlane = "org.yaoqiang.bpmn.model.elements.bpmndi.BPMNPlane"
			.hashCode(); // Useless

	public static final int BPMNProcess = "org.yaoqiang.bpmn.model.elements.process.BPMNProcess"
			.hashCode();

	public static final int BPMNShape = "org.yaoqiang.bpmn.model.elements.bpmndi.BPMNShape"
			.hashCode();

	public static final int CallActivity = "org.yaoqiang.bpmn.model.elements.activities.CallActivity"
			.hashCode();
	public static final int Collaboration = "org.yaoqiang.bpmn.model.elements.collaboration.Collaboration"
			.hashCode();
	public static final int Documentation = "org.yaoqiang.bpmn.model.elements.core.foundation.Documentation"
			.hashCode();
	public static final int eDefinitions = "be.fnord.yaoqiang.extend.eDefinitions"
			.hashCode();
	public static final int EndEvent = "org.yaoqiang.bpmn.model.elements.events.EndEvent"
			.hashCode();
	public static final int ErrorEventDefinition = "org.yaoqiang.bpmn.model.elements.events.ErrorEventDefinition"
			.hashCode();
	public static final int ExclusiveGateway = "org.yaoqiang.bpmn.model.elements.gateways.ExclusiveGateway"
			.hashCode();
	public static final int InclusiveGateway = "org.yaoqiang.bpmn.model.elements.gateways.InclusiveGateway"
			.hashCode();
	public static final int IntermediateCatchEvent = "org.yaoqiang.bpmn.model.elements.events.IntermediateCatchEvent"
			.hashCode();
	public static final int IntermediateThrowEvent = "org.yaoqiang.bpmn.model.elements.events.IntermediateThrowEvent"
			.hashCode();
	public static final int MessageFlow = "org.yaoqiang.bpmn.model.elements.collaboration.MessageFlow"
			.hashCode();
	public static final int ParallelGateway = "org.yaoqiang.bpmn.model.elements.gateways.ParallelGateway"
			.hashCode();
	public static final int Participant = "org.yaoqiang.bpmn.model.elements.collaboration.Participant"
			.hashCode();
	private static String ProcessName = "";
	public static final int SequenceFlow = "org.yaoqiang.bpmn.model.elements.core.common.SequenceFlow"
			.hashCode();
	public static final int ServiceTask = "org.yaoqiang.bpmn.model.elements.activities.ServiceTask"
			.hashCode();
	public static final int StartEvent = "org.yaoqiang.bpmn.model.elements.events.StartEvent"
			.hashCode();
	public static final int SubProcess = "org.yaoqiang.bpmn.model.elements.activities.SubProcess"
			.hashCode();
	public static final int Task = "org.yaoqiang.bpmn.model.elements.activities.Task"
			.hashCode();
	...
	public static final int UserTask = "org.yaoqiang.bpmn.model.elements.humaninteraction.UserTask"
			.hashCode();

	/**
	 * Convert a yaoqiang bpmn node to a simple Vertex Try to use actual mapping
	 * but if can't figure it out try to use basic element names
	 *
	 * @param inputNode
	 * @return
	 */
	public static Vertex convertNode(
			org.yaoqiang.bpmn.model.elements.core.common.FlowNode inputNode) {
		...
		if (outputNode == null) {
			if (type.toLowerCase().contains("gateway")) {
				...
				outputNode = new Vertex("" + inputNode.getName(),
						GraphLoader.ExclusiveGateway);
	...
	public static String getType(int _type) {
		...
		if (_type == ParallelGateway) {
			return "ParallelGateway";
		}
		if (_type == InclusiveGateway) {
		...
		if (_type == IntermediateCatchEvent) {
			return "IntermediateCatchEvent";
		}
		if (_type == IntermediateThrowEvent) {
			return "IntermediateThrowEvent";
		}
		if (_type == BoundaryEvent) {
	...
	public Graph<Vertex, Edge> loadModel(String filename, int flags) {
		...
		document = BPMNModelUtils.parseDocument(filename, true,
				new LinkedList<ErrorMessage>());
		...
		if (document != null) {
			...
			// Change the dont save messages and participants flag to true if
			// you want pools and messages
			for (String key : myCodec.getBPMNElementMap().keySet()) {
				saveElement(myCodec.getBPMNElementMap().get(key), flags, "");
			}
			...
			for (String key : tEdges.keySet()) {
				...
				if (!tNodes.containsKey(r.src)) {
					System.out.println("Unable to find vertex reference ID "
							+ r.src);
					continue;
				}
				;
				...
				;
				// Create a new edge
				Edge e = new Edge(key + r.name, r.type + "",
						tNodes.get(r.src).node, tNodes.get(r.trg).node);
				myModel.addE(e);
			}
	...
	/**
	 * Updated converter function for all XMLElements used when processing value
	 * elements of myCodec.getBPMNElementMap()
	 */
	public void saveElement(XMLElement _ele, int getMessages,
		...
		if (hashCode == Documentation) {
			...
			if (__DEBUG) {
				a.e.println("documentation " + " " + r.getId() + " "
						+ r.toValue() + " saving to " + et.getId());
			}
			if (tNodes.containsKey(et.getId())) {
				TNode at = tNodes.get(et.getId());
				tNodes.remove(et.getId());
				at.node.addWFF(r.toValue());
				tNodes.put(et.getId(), at);
			} else {
				saveElement(et, getMessages, r.toValue());
			}
			return;
		}
		...
		if (hashCode == ExclusiveGateway) {
			...
			TNode myNode = new TNode(r.getId(), r.getName(), ExclusiveGateway,
					_ele);
		...
		if (hashCode == ParallelGateway) {
			...
			TNode myNode = new TNode(r.getId(), r.getName(), ParallelGateway,
					_ele);
		...
		if (hashCode == InclusiveGateway) {
			...
			TNode myNode = new TNode(r.getId(), r.getName(), InclusiveGateway,
					_ele);
		...
		if (hashCode == ErrorEventDefinition) {
			...
			TNode myNode = new TNode(r.getId(), r.toName(),
					ErrorEventDefinition, _ele);
		...
		if (hashCode == SequenceFlow) {
			...
			TEdge myEdge = new TEdge(r.getId(), r.getName(), r.getSourceRef(),
					r.getTargetRef(), SequenceFlow, _ele);
			tEdges.put(r.getId(), myEdge);
			// a.e.println("Flow " + " " + r.getId() + " " + r.getName() +
			// " between " + r.getSourceRef() + " / " +
			// r.getSourceFlowNode().getId());
			return;
		}
		if (hashCode == Collaboration) {
			// Collaboration r = (Collaboration)_ele;
			// a.e.println("Collaboration " + " " + r.getId() + " " +
			// r.getConversationNodes().toElements().get(0).toValue());
			// Doesn't seem important?
			return;
		}
		if (hashCode == Task) {
		...
		if (hashCode == Participant) {
			if ((getMessages & a.e.DONT_SAVE_MESSAGES_AND_PARTICIPANTS) > 0) {
				return;
			}
		...
		if (hashCode == IntermediateCatchEvent) {
			...
			TNode myNode = new TNode(r.getId(), r.getName(),
					IntermediateCatchEvent, _ele);
			Vertex v = new Vertex(r.getId() + r.getName(),
		...
		if (hashCode == BoundaryEvent) {
			...
			TNode myNode = new TNode(r.getId(), r.getName(), BoundaryEvent,
					_ele);
		...
		a.e.println("Don't have a handler in the graphLoader for elements of type "
				+ _ele.getClass().getCanonicalName());
...
class TEdge {
	...
	public TEdge(String _id, String _name, String _src, String _trg, int _type,
			XMLElement _ele) {
		id = _id;
		name = _name;
		src = _src;
