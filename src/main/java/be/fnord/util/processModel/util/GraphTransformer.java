...
/**
 * Transformation functions for graphs TODO Fix the templating system, I've
 * written over it to produce code very quickly, if you can figure out what the
...
public class GraphTransformer {
	...
	public LinkedList<Trace> createTrace(Graph<Vertex, Edge> g) {
		...
		// All input models should be decision free models, so we need to first
		// check for parallel gateways.
		// Break down the model into sections of parallel and non parallel paths
		// Then combine each non-parallel path with OCP instances of the
		// parallel path.
	...
	private void createTrace(Graph<Vertex, Edge> g, Trace currentTrace,
			Vertex currentNode, Vertex compareEnd) {

		currentTrace.addTraceNode(currentNode); // Add the current node to the
												// trace
		...
		if (g.outDegreeOf(currentNode) > 1 && currentNode.isAND) {
			...
			if (__DEBUG) {
				a.e.println("Splitting into parallel at " + currentNode
						+ " until " + currentNode.corresponding);
			}
	...
	/**
	 * Create a hash for a graph. Function can be configured by setting
	 * DEDUPING_LEVEL in a.e class. Not a great method and could use some
	 * improvement in the future
	 *
	 * @param in
	 * @return String representing a hash of a particular graph based on the
	 *         edges and nodes.
	 */
	public String hashDecisionFreeGraph(Graph<Vertex, Edge> in) {
		...
		if (in == null) {
			return "NULLLLLLLLLED";
		}
		for (Vertex v : in.vertexSet()) {
			vertices.add((v.toString().length() > 3 ? v.toString().substring(0,
					v.toString().length()) : v.toString().substring(0, 4))
					.trim());
		}
		for (Edge e : in.edgeSet()) {
			edges.add((e.id.toString().length() > 3 ? e.id.toString()
					.substring(0, e.id.toString().length()) : e.id.toString()
					.substring(0, 4)).trim());
		}
	...
	/**
	 * Create a set of decision free process model from an input model. This
	 * will split models to have only one start event etc.
	 *
	 * @param g
	 *            Input graph
	 * @return LinkedList<PGraph<Vertex,Edge>>
	 */
	public LinkedList<Graph<Vertex, Edge>> makeDecisionFree(
			Graph<Vertex, Edge> g) {
		...
		if (boundaryCounter > 1) {
			continuer = true;
		}

		if (!continuer) { // No XOR's or many starts found, nothing to do so
							// lets add the graph to the return
		...
		if (startCounter > 1) {
			...
			// Send each of the new subgraphs to the function again to deal with
			// any and all XOR's
		...
		for (String startID : startEvents) {
			...
			for (String gateID : splitGates) {
				...
				if (g.vertexIDRef.get(gateID) == null) {
					if (__DEBUG) {
						a.e.println("Can't find gateway " + gateID
								+ " skipping fragmenting");
					}
					continue;
				}
				...
				if (__DEBUG) {
					a.e.println("Gateway: " + gateway.toString());
				}

				FloydWarshallShortestPaths<Vertex, Edge> pather = new FloydWarshallShortestPaths<Vertex, Edge>(
						g);
				...
				if (gp != null) {
					...
					for (Vertex successor : succGates) {
						...
						Graph<Vertex, Edge> pg = new Graph<Vertex, Edge>(
								Edge.class);
						...
						Graph<Vertex, Edge> pg2 = new Graph<Vertex, Edge>(
								Edge.class);
						...
						// Replaced predGate with gateway to try to include
						// gateway node
						result.addAll(merge(startFragment, endFragment,
								predGate, gateway, successor));
					}
				} else {
					if (__DEBUG) {
						a.e.println("GP was null for " + " from " + startEvent
								+ " to " + gateway);
					}
				}
			}
	...
	public LinkedList<Graph<Vertex, Edge>> merge(
			LinkedList<Graph<Vertex, Edge>> startFrags,
			LinkedList<Graph<Vertex, Edge>> endFrags, Vertex endOfPred,
			Vertex gateway, Vertex startOfSucc) {
	 ...
	 * @return
	 */
	public LinkedList<Graph<Vertex, Edge>> removeDupesFromDecisionFreeGraphs(
			LinkedList<Graph<Vertex, Edge>> duped) {

		if (a.e.DEFAULT_DEDUPING_LEVEL == a.e.NO_DEDUPING) {
			return duped;
		}
		...
		for (Graph<Vertex, Edge> g : duped) {
			...
			if (!hashedDecisionFreeGraphs.contains(key)) {
				...
				a.e.println("Found duped graph " + key);
			}
		}
	...
	public Graph<Vertex, Edge> seqComp(Graph<Vertex, Edge> lhs,
			Graph<Vertex, Edge> rhs) {
