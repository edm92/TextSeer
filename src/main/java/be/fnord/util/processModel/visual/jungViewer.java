...
public class jungViewer {
	...
	public void displayGraph(be.fnord.util.processModel.Graph<Vertex, Edge> g,
			String name) {
		...
		for (Edge e : g.edgeSet()) {
			vg.addEdge(e.id.toString(), e.getSource().toString(), e.getTarget()
					.toString());
		}
		...
		BasicVisualizationServer<String, String> vv = new BasicVisualizationServer<String, String>(
				layout);
		vv.setPreferredSize(new Dimension(800, 800)); // Sets the viewing area
														// size
