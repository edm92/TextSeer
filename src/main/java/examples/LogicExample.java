...
/**
 * The following code demonstrates the logic engine powered mainly by Orbital
 * Library
 * <p/>
 * <p/>
 * Example Output (p->q), (m-> p V q)} |= (m->q)? true (p->q), (m-> p V q)} U
 * ~(m->q)? true Checking if ~(a & b) | ~c is consistent : true Checking if ~(a
 * & b) | ~c entails (a & b) -> ~c : true Effect scenario resulting from acc:
 * (~(a & b) | ~c) & ((a & b) -> ~c) Checking if (~(a & b) | ~c) & ((a & b) ->
 * ~c) is consistent : true Effect scenario resulting from acc: (a & b) Checking
 * if (a & b) is consistent : true Execution time was 424 ms.
 *
...
// Effect e5 = new
// Effect(" (~a | ~b | c) & ~(~c | d) & (~c | e) & (a & b -> c) & (~e | ~f | g) & ((c & d) -> e) & (c & f -> g) & (a | d) & (f & g) & (b | ~a) & (p -> q) & ((l & m) -> p) & ((b & l) -> m) & ((a & p) -> l) & ((a & b) -> l)");
public class LogicExample {
	...
	public static void main(String[] Args) {
		...
		// Check if consistent
		System.out.println("Checking if " + e1 + " is consistent : "
				+ e1.issat());
		System.out.println("Checking if " + e1 + " entails " + e2 + " : "
				+ e1.entails(e2));
		...
		for (WFF e : resultEffects) {
			...
			System.out.println("Checking if " + e + " is consistent : "
					+ e.issat());
		...
		resultEffects = acc
				.pairwise_acc(e6, e7, "((a & b) -> (~Teste2))", true);
